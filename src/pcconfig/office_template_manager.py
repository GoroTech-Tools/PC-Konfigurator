...
class OfficeTemplateManager:
    @staticmethod
    def _configure_word_list_indents_via_com(template_path: Path) -> bool:
        """Speichert die Standard-Einzüge für Aufzählungs-/Nummerierungslisten."""
        import win32com.client  # type: ignore

        backup_path = template_path.with_name(template_path.name + ".pckconfig-list-backup")
        ...
        try:
            ...
            # Word arbeitet bei ListLevel-Positionen mit Punkt. 0,5 cm sind
            # 14,173 pt. Dokumentgebundene ListTemplates werden verwendet,
            # damit Word die Definitionen tatsächlich in Normal.dotm bzw.
            # NormalEmail.dotm speichert; reine Änderungen an ListGalleries
            # bleiben häufig nur anwendungsweit und gehen beim nächsten Start
            # verloren.
            half_cm = 72.0 / 5.08
            bullet_template = document.ListTemplates.Add(OutlineNumbered=True)
            number_template = document.ListTemplates.Add(OutlineNumbered=True)
            for level in range(1, 10):
                marker_position = (level - 1) * half_cm
                text_position = level * half_cm

                bullet_level = bullet_template.ListLevels(level)
                bullet_level.NumberFormat = "•"
                bullet_level.NumberPosition = marker_position
                bullet_level.TextPosition = text_position
                bullet_level.TabPosition = text_position

                number_level = number_template.ListLevels(level)
                number_level.NumberStyle = 0  # wdListNumberStyleArabic
                number_level.NumberFormat = f"%{level}."
                number_level.NumberPosition = marker_position
                number_level.TextPosition = text_position
                number_level.TabPosition = text_position

            # Zusätzlich die aktuell verwendeten Word-Galerien anpassen. Die
            # Symbolleistenbefehle für Aufzählung/Nummerierung greifen häufig
            # auf diese globalen Vorlagen statt auf benutzerdefinierte Styles
            # zurück.
            for gallery_id in (1, 2):  # wdBulletGallery, wdNumberGallery
                gallery = word.ListGalleries.Item(gallery_id)
                for template_index in range(1, gallery.ListTemplates.Count + 1):
                    gallery_template = gallery.ListTemplates.Item(template_index)
                    try:
                        gallery_level = gallery_template.ListLevels(1)
                        gallery_level.NumberStyle = 23 if gallery_id == 1 else 0
                        marker_position = 0
                        text_position = half_cm
                        gallery_level.NumberPosition = marker_position
                        gallery_level.TextPosition = text_position
                        gallery_level.TabPosition = text_position
                    except Exception:
                        continue

            # Der eingebaute Word-Style „Listenabsatz“ kann zusätzlich eigene
            # Absatz-Einzüge tragen. Word zeigt sonst bei konkreten
            # Listenabsätzen trotz passender ListTemplate weiterhin den
            # Standardwert von ca. 0,63 cm an. WdBuiltinStyle -180 ist
            # versionsübergreifend der Style „List Paragraph“ bzw.
            # „Listenabsatz“.
            try:
                list_paragraph_style = document.Styles(-180)
                list_paragraph_style.ParagraphFormat.LeftIndent = 0
                list_paragraph_style.ParagraphFormat.FirstLineIndent = 0
                logging.getLogger(__name__).info(
                    "Word-Style Listenabsatz neutral gesetzt: linker Einzug 0 cm, kein eigener hängender Einzug"
                )
            except Exception as style_error:
                logging.getLogger(__name__).info(
                    "Word-Style Listenabsatz konnte nicht angepasst werden: %s",
                    style_error,
                )

            # Die eingebauten List Bullet/List Number-Stile werden damit an
            # die dokumentgebundenen Definitionen gekoppelt. Ist eine
            # lokalisierte Word-Version im Einsatz, schlägt die direkte
            # Stilverknüpfung fehl; die gespeicherten ListTemplates bleiben
            # trotzdem erhalten und der Vorgang gilt nicht als beschädigt.
            for style_id, list_template in (
                (-49, bullet_template),  # Aufzählungszeichen/List Bullet
                (-50, number_template),  # Listennummer/List Number
            ):
                try:
                    document.Styles(style_id).LinkToListTemplate(list_template, 1)
                except Exception as style_error:
                    logging.getLogger(__name__).info(
                        "Listenstil %s konnte nicht direkt verknüpft werden: %s",
                        style_id,
                        style_error,
                    )

            document.Save()
            # Die Befehle im Word-Ribbon verwenden die globale Galerie aus
            # Normal.dotm. Änderungen an Application.ListGalleries werden
            # nicht zuverlässig mit dem geöffneten Dokument persistiert.
            # Deshalb die globale NormalTemplate-Galerie explizit speichern,
            # aber nur beim Word-Template selbst (nicht beim Outlook-Template).
            if template_path.name.lower() == "normal.dotm":
                try:
                    normal_template = word.NormalTemplate
                    normal_template.Saved = False
                    normal_template.Save()
                    logging.getLogger(__name__).info(
                        "Globale Word-Listengalerien in Normal.dotm gespeichert"
                    )
                except Exception as normal_save_error:
                    logging.getLogger(__name__).warning(
                        "Globale Word-Listengalerien konnten nicht gespeichert werden: %s",
                        normal_save_error,
                    )
            ...
            if not SafeTemplateProcessor.normalize_word_list_indents_xml(template_path):
                raise ValueError("Word-Listen-Einzüge konnten nicht gespeichert werden")
            with zipfile.ZipFile(template_path, "r") as archive:
                if archive.testzip() is not None:
                    raise ValueError("Word-Vorlage nach Listeneinzug-Speicherung beschädigt")
            return True
        except Exception:
    ...
    def copy_templates_to_user(self):
        ...
        try:
            ...
            # Die Listengalerien werden von Word nicht in einer separaten
            # numbering.xml der minimalistischen Vorlagen abgelegt. Deshalb
            # nach dem Kopieren direkt in Normal.dotm und NormalEmail.dotm
            # persistieren. Scheitert COM, bleiben die übrigen Template-Schritte
            # gültig und der Hinweis wird nur protokolliert.
            for template_key in ("normal_dotm", "normal_email_dotm"):
                if not results.get(template_key):
                    continue
                template_path = self.target_paths.get(template_key)
                try:
                    if self._configure_word_list_indents_via_com(Path(template_path)):
                        self.logger.info(
                            "Word-Listeneinzüge gespeichert: %s (0,5-cm-Raster)",
                            template_path,
                        )
                except Exception as exc:
                    self.logger.warning(
                        "Word-Listeneinzüge konnten für %s nicht per COM gespeichert werden: %s",
                        template_path,
                        exc,
                    )

...
if (Confirm-OfficeClosure) {
    ...
    if ($useDocuments) {
        ...
        $defaultOfficeSavePath = $UserdocPath
        ...
        $defaultOfficeSavePath = $driveRoot
    ...
    $global:DefaultOfficeSavePath = $defaultOfficeSavePath
    ...
    function Set-OfficeRegistrySettings {
        ...
        # Dynamische Settings je nach Auswahl
        $wordSettings = @{
            "DeveloperTools" = 1
            "Ruler" = 1
            "ShowAllFormatting" = 1
            "VisiDrawTableDrs" = 1
            "DOC-PATH" = $global:DefaultOfficeSavePath
        ...
        $excelSettings = @{
            "DeveloperTools" = 1
            "DefaultPath" = $global:DefaultOfficeSavePath
            "PersonalTemplates" = $global:BackupTargetPath
            "DisableBootToOfficeStart" = 1
    ...
    if ($FontName -eq "Aptos Narrow") {
        # Versuche verschiedene mögliche Namen für Aptos Narrow
        ...
        # Standard-Name verwenden, Word wird automatisch den besten Match finden
        $ActualFontName = "Aptos Narrow"
    }

    Write-Log "Gewählte Schriftart: $FontName (Verwendet: $ActualFontName)" "INFO"

    # Benutzerabfrage: Auswahl der Schriftgröße für Word/Outlook
    ...
    # Benutzerabfrage: Auswahl der Schriftgröße für Excel
    ...
    try {
        ...
        try {
            ...
            $word.Visible = $false
            $template = $word.Templates.Open($normalDotm)
            $style = $template.Styles.Item("Standard")
            $style.Font.Name = $fontName
            $style.Font.Size = $fontSize
            $template.Save()
            $template.Close()
            $word.Quit()
            $success = $true
            Write-Host "Normal.dotm erfolgreich per COM-Automation angepasst." -ForegroundColor Green
        } catch {
            Write-Host "COM-Automation für Word fehlgeschlagen: $($_.Exception.Message)" -ForegroundColor Yellow
            $success = $false
        }

        if (-not $success) {
            try {
                Copy-Item -Path $fallbackDotm -Destination $normalDotm -Force
                Write-Host "Fallback: Vorlage aus Datei-Vorlagen\\Sonstiges\\Standards kopiert." -ForegroundColor Cyan
            } catch {
                Write-Host "Fehler beim Kopieren der Fallback-Vorlage: $($_.Exception.Message)" -ForegroundColor Red
            }
        }
    } catch {

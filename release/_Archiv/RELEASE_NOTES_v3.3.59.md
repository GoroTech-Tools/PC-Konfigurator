# Release Notes v3.3.59

Datum: 2026-09-11

## Highlights

- Namens-, Build- und Doku-Anpassungen wurden in diesem Release-Stand konsolidiert.
- Das Build wurde als Onefile-EXE erzeugt und für die Verteilung aufbereitet.
- Bitte Highlights bei Bedarf projektspezifisch ergänzen.

## Qualitätsstatus

- Release-Build erfolgreich erzeugt.
- ZIP-Artefakt erstellt und im Release-Ordner abgelegt.
- Automatische Basisprüfung (Build/Packaging) im Skript durchlaufen.

## Artefakte

- Build-Verzeichnis: dist/PC-Konfigurator-v3.3.59/
- EXE: dist/PC-Konfigurator-v3.3.59/PC-Konfigurator.exe
- Release-ZIP: release/PC-Konfigurator-v3.3.59.zip

## Enthaltene Commits (aktuelle Historie)

- `0496d8d` release-v3.3.50
- `b0d1600` fix: Konfigurationstab beim Moduswechsel bereinigen
- `9454b6b` fix: Word- und Outlook-Post-Verify getrennt prüfen
- `5fb56a6` feat: einfache und erweiterte Laufmodi trennen
- `5288fd7` feat: separate Outlook font size with 12pt default

## Technische Build-Informationen

- Build-Datum: 2026-09-11 17:35:46
- Build-Modus: --onefile --windowed
- EXE-Name: PC-Konfigurator.exe

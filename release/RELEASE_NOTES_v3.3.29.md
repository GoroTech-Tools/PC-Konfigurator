# Release Notes v3.3.29

Datum: 2026-07-03

## Highlights

- Namens-, Build- und Doku-Anpassungen wurden in diesem Release-Stand konsolidiert.
- Das Build wurde als Onefile-EXE erzeugt und für die Verteilung aufbereitet.
- Bitte Highlights bei Bedarf projektspezifisch ergänzen.

## Qualitätsstatus

- Release-Build erfolgreich erzeugt.
- ZIP-Artefakt erstellt und im Release-Ordner abgelegt.
- Automatische Basisprüfung (Build/Packaging) im Skript durchlaufen.

## Artefakte

- Build-Verzeichnis: dist/PC-Konfigurator-v3.3.29/
- EXE: dist/PC-Konfigurator-v3.3.29/PC-Konfigurator.exe
- Release-ZIP: release/PC-Konfigurator-v3.3.29.zip

## Enthaltene Commits (aktuelle Historie)

- `6e22f32` Release v3.3.25: GUI-Workflow, Mermaids und Doku aktualisiert
- `726cd4d` Release v3.3.23: update docs, build flow and release notes
- `eec897e` Release v3.3.22: build artifacts
- `cea6602` Release v3.3.22: font hardening and Windows defaults
- `bdb5081` Build: add two-step markdown quickcheck (fix + gate)

## Technische Build-Informationen

- Build-Datum: 2026-07-03 20:35:31
- Build-Modus: --onefile --windowed
- EXE-Name: PC-Konfigurator.exe
